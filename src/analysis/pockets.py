...
def batch_analyze_state(state_name, ensemble_path, pdb_root, analysis_root, n_structures=100):
    ...
    for i in range(n_structures):
        pdb_path = os.path.join(pdb_dir, f"structure_{i:03d}.pdb")
        export_to_pdb(coords[i], pdb_path)
        
        # P2Rank output structure: out_dir/structure_000.pdb_predictions.csv
        # We need to pass the state-specific out_dir
        run_p2rank(pdb_path, out_dir)
        
        # Parse
        p_res = parse_p2rank_output(out_dir, f"structure_{i:03d}.pdb")
        results.append({
            "id": i,

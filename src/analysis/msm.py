...
def run_ck_test(latent_trajectories: list, lag_time: int, n_samples: int = 100):
    ...
    Uses a BayesianMSM so the CK test carries error estimates (95% CI), which is
    what makes the diagonal-overlap comparison meaningful. A MaximumLikelihoodMSM
    yields point estimates only and cannot support a pass/fail judgement.
    """
    ...
    # 2. Fit Bayesian MSMs at lag multiples 1..5
    models = [
        BayesianMSM(lagtime=lag_time * i, n_samples=n_samples).fit_fetch([dtraj])
        for i in range(1, 6)
    ]

    # 3. CK test with error estimation
...
def fit_reversible_msm(dtrajs: np.ndarray, lag: int, reversible: bool = True):
    """Fit a maximum-likelihood reversible MSM on a discrete trajectory (1D integer array)."""
    from deeptime.markov.msm import MaximumLikelihoodMSM

    return MaximumLikelihoodMSM(lagtime=int(lag), reversible=reversible).fit_fetch(dtrajs)
